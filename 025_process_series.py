...
@app.cell
def _(args, dicom2nifti, pathlib, process_nifti_series, series_path):
    input_path = None
    ...
    if input_path:
        if input_path.is_dir():
            ...
            temp_nifti = str(args.zarr_name) + ".nii"
            dicom2nifti.dicom_series_to_nifti(str(input_path), temp_nifti)
            processed_data = process_nifti_series(pathlib.Path(temp_nifti))
            processed_data[3]["raw_path"] = str(input_path)
            pathlib.Path(temp_nifti).unlink()
...
def _(itertools, nib, np, pathlib):
    def _calculate_physical_extents(data_shape: tuple, affine: np.ndarray) -> tuple[float, float, float]:
        ...
        corners_vox = np.array(list(itertools.product([0, w - 1], [0, h - 1], [0, d - 1])))
    ...
    def process_nifti_series(nifti_path: pathlib.Path) -> dict | None:
        ...
        try:
            ...
            final_data = original_img.get_fdata(dtype=np.float32)
            final_affine = original_img.affine
            ...
            return final_data, final_affine, original_img.header, {
                "patient_id": "unknown",
                "study_uid": "unknown",
                "series_uid": "unknown",
                "modality": "unknown",
                ...
                "shape_RL": final_data.shape[0],
                "shape_PA": final_data.shape[1],
                "shape_IS": final_data.shape[2],
                "px_shape_RL": zooms[0],
                "px_shape_PA": zooms[1],
                "px_shape_IS": zooms[2],
                "wc": -666.0,
